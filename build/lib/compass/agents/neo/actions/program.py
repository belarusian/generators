...
Invoke Programmer NFA for complex code generation.
...
def _(
    ...
    """
    Execute program action via Trinity.

    Trinity plans the work -- read files, compute, shell, write files --
    and can optionally delegate to the Programmer NFA via
    artifact_type="programmer" for heavy multi-file generation.
    ...
    """
    from compass.generators._types import Ok as GenOk, Err as GenErr
    from compass.generators.trinity.generate import run as trinity_run
    ...
    match result:
        case GenOk(value):
            # value is the output Path -- read facts from result.json
            output_dir = Path(value) if not isinstance(value, Path) else value
            ...
            if result_file.exists():
                ...
                try:
                    ...
                    for f in data.get("facts", []):
                        name = f.get("name", "?")
                        val = str(f.get("value", ""))[:100]
                        facts_summary.append(f"  [{name}]: {val}")
                except Exception:
                    pass
            summary = f"Trinity completed: {len(facts_summary)} facts produced"
            if facts_summary:
                summary += "\n" + "\n".join(facts_summary)
            return True, summary
        case GenErr(error=e):
            return False, f"Trinity failed: {e}"
