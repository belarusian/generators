...
def parse_servers(servers_str: str) -> Dict[str, str]:
    ...
    Example: "local=http://localhost:11434,big=http://remote-ollama:11434"
...
def get_provider_by_id(provider_id: str) -> Provider:
    ...
    if provider_id.lower().startswith("llamacpp"):
        ...
        if os.getenv("LLAMACPP_HOST"):
            llamacpp_servers.setdefault("local", os.getenv("LLAMACPP_HOST"))
        if "@" in provider_id:
    ...
    if os.getenv("OLLAMA_HOST"):
        servers.setdefault("local", os.getenv("OLLAMA_HOST"))
