...
_INNER_MODULE_CONTRACT = """\
Every generated generator must match the exemplar's parse contract:

- `_types.py` MUST define `validate_spec_instance(spec: Spec) -> Result[Spec, str]`
  for that module's `Spec` dataclass. The generated `generate.py` imports this
  from `._types` in `_make_parse` (same pattern as `compass/generators/meta` and Trinity).
- If you only define `Spec` in `_types.py`, also add `validate_spec_instance` that
  returns `Ok(spec)` after basic checks, or `Err(...)` with a clear message.
"""


def _build_inner_contract_section() -> DomainSection:
    return DomainSection(
        heading="Required inner module contract",
        content=_INNER_MODULE_CONTRACT,
    )
...
def build_meta_context(
    ...
    contract_section = _build_inner_contract_section()
    exemplar_section = _build_exemplar_context(gen_root)

    # Filter out empty sections
    sections = tuple(
        s for s in (framework_section, contract_section, exemplar_section)
