...
CYCLE_BREAKING = True  # Generator: LLM-dependent, non-deterministic output
...
class FactsRef:
    """Mutable reference for session facts, updated each REPL turn.

    Same pattern as ModelRef -- closures capture the object, REPL
    updates .facts before each generation so prior session facts
    are available at execution time.
    """
...
def _make_parse(on_progress=None):
    """V1_trinity: Spec -> Result[Spec] (instance validation after parse)."""
...
def _make_validate(model_ref: ModelRef, ask_fn=None, workspace: Path | None = None, on_progress=None, facts_ref: FactsRef | None = None):
    """V2_trinity: Spec -> Result[ExecutionResult] | Cycle

    Composed cheapest-first:
      1. Semantic: inline Python syntax, artifact refs
      2. Executive: run the plan, collect facts

    facts_ref: mutable reference updated by REPL each turn so prior
    session facts are available at execution time.

    Cycle-break facts accumulate across rounds so that re-planned
    specs skip already-completed non-deterministic steps and their
    facts are available as $fact references.
    """
    _cycle_facts: dict = {}

    def validate(spec: Spec) -> Result:
        # Merge: session facts (REPL) + cycle-break facts
        pf = dict(_cycle_facts)
        if facts_ref and facts_ref.facts:
            pf.update(facts_ref.facts)

        result = validate_plan(
            spec, model_ref.model_id, ask_fn, workspace,
            on_step=on_progress, prior_facts=pf or None,
        )

        # Extract facts from Cycle for next iteration's prior_facts
        if isinstance(result, Cycle):
            _cycle_facts.update(result.facts)
    ...
    *,
    ...
    if live:
        ...
        return trinity_repl(
            ...
            validate=_make_validate(model_ref, ask_fn, ws, on_progress, facts_ref),
