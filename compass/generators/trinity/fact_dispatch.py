...
def resolve_fact(fact) -> Any:
    ...
    If raw_value is set (type-contracted facts), return it directly.
    Otherwise fall back to json.loads -> raw string (backward compat).
    """
    if getattr(fact, "raw_value", None) is not None:
