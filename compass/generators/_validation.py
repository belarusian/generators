...
Used by any generator that produces Python source (notebooks, CLI tools,
modules, etc.).
"""
...
class DefCollector(ast.NodeVisitor):
    ...
    Increments conditional depth inside if and try blocks.
    Assignments at depth > 0 are marked conditional.
    """

    def __init__(self) -> None:
        self.defs: dict[str, bool] = {}  # name -> conditional
        self._depth = 0
...
def validate_python_sources(
    ...
    *,
    ...
    Pure function (ast only). Operates on raw strings -- no notebook types.
    The label parameter controls error message formatting (e.g. "cells", "files").

    Returns Ok(None) if all sources are valid, Err(description) otherwise.
    """
    errors: list[str] = []
...
def summarize_variable_flow(
    ...
    *,
    ...
    """Produce a human/model-readable summary of cross-source variable flow.

    For each source, lists:
    - Names defined unconditionally (available to all later sources)
    - Names defined conditionally (may cause NameError in later sources)
    - Names referenced from prior sources (dependencies)
    """
    env: dict[str, _VarDef] = {}
    ...
    for i, source in enumerate(sources):
        ...
        unconditional = sorted(
            n for n, cond in cell_defs.items() if not cond
        )
        conditional = sorted(
            n for n, cond in cell_defs.items() if cond
        )
