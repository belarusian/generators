...
def _init_backend():
    ...
    # Try Ollama first (prefers @local -- keep big free for Oracle)
    if _test_ollama():
        _backend = "ollama"
        return

    raise RuntimeError(f"No embedding backend available. Configure OLLAMA_SERVERS with {OLLAMA_EMBED_MODEL}.")
...
def _embed_texts(texts: list) -> np.ndarray:
    ...
    For Ollama: tries batch endpoint first (batches of 32, parallel workers),
...
def _embed_query(query: str) -> np.ndarray:
    """Embed a single query. Cached with FIFO eviction."""
    global _query_cache
...
class CodeEmbedder:
    ...
    def _embed_chunks(self, changed_chunk_ids: Optional[Set[str]] = None):
        ...
        if old_lookup:
            ...
            self.embeddings = _embed_texts(texts)
