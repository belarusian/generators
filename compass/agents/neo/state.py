...
# ============================================================================
# EAS (Effect Action System) State Machine for Local-First Agent Workflows
# ============================================================================

class EASState:
    """States in the EAS (Effect Action System) workflow."""
...
EAS_TRANSITIONS = [
    {"from": EASState.INITIALIZED, "event": "start_generation", "to": EASState.GENERATING},
    {"from": EASState.GENERATING, "event": "start_validation", "to": EASState.VALIDATING},
    {"from": EASState.VALIDATING, "event": "validation_passed", "to": EASState.COMPLETED},
    {"from": EASState.VALIDATING, "event": "validation_failed", "to": EASState.FIXING},
    {"from": EASState.FIXING, "event": "fix_successful", "to": EASState.VALIDATING},
    {"from": EASState.FIXING, "event": "max_fixes_exceeded", "to": EASState.EXHAUSTED},
]

def get_eas_nfa_state_machine():
    """Get the EAS NFA state machine for local-first agent workflows."""
    return {
        "transitions": EAS_TRANSITIONS,
        "initial_state": EASState.INITIALIZED,
        "final_states": [EASState.COMPLETED, EASState.EXHAUSTED],
    }
