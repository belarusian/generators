...
def build_eval_context(response_type: Type) -> Dict[str, Any]:
    """
    Build the eval() context with all needed types.

    Includes the response type and all its dependencies.
    """
    context = {}
...
def validate_python_block(code: str) -> tuple[bool, Optional[str]]:
    """
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            return False, f"Import statements not allowed: {ast.dump(node)}"
...
def parse_typed_response(
    ...
    # Validate it's safe Python
    is_safe, safety_error = validate_python_block(code)
    if not is_safe:
        raise ValueError(f"Invalid Python: {safety_error}")

    # Exec in a restricted namespace with all needed types
    eval_context = build_eval_context(response_type)
...
def parse_response_with_files(
    ...
    meta = parse_typed_response(constructor_text, response_type)
